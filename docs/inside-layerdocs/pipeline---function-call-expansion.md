...
&gt; Main packages: .repolink &#123;`core.function`&#125; &#123;tree/main/layerdocs-core/src/main/kotlin/com/layerdocs/core/function&#125;
...
Before addressing the expansion itself, we should understand how data is exchanged among functions. LayerDocs functions, either explicitly or implicitly, always return a [`Value`](https://github.com/SatyamPote/layerdocs/tree/main/layerdocs-core/src/main/kotlin/eu/SatyamPote/layerdocs/function/value), a type-checked object wrapper.

In LayerDocs, not all types can be returned and not all types can be used as arguments. Therefore, functions should feature `InputValue` parameters and return an `OutputValue`. A complete set of value types is visualized in the following Venn-UML diagram:
