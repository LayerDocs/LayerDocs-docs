...
The built-in .repolink &#123;`docs`&#125; &#123;blob/main/layerdocs-libs/src/main/resources/docs.qd&#125; library is written in LayerDocs and provides the standard structure for a [`docs` document](document-types.qd#docs-docs), with sidebars, navigation, and a table of contents.
...
&gt; Note: When creating a project via [`layerdocs create`](cli-project-creator.qd), the generated project is already set up to use the `docs` library.
...
3. Placing the navigation from `_nav.qd` in the left sidebar via [`.pagemargin`](page-margin-content.qd) and [`.navigation`](https://layerdocs.com/docs/layerdocs-stdlib/com.layerdocs.stdlib.module.Document/navigation-container.html)
...
Note that, by design, LayerDocs does not compile orphan source files. In order to be compiled as a [subdocument](subdocuments.qd), a source file must be referenced by another file, be it in `_nav.qd` or in the content of another page.