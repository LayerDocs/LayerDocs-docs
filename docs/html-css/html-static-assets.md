...
.docname &#123;HTML static assets&#125;
.include &#123;docs&#125;

Static assets let you ship arbitrary files alongside your compiled HTML output, without any further processing.

Place files in a `public/` directory at the project root, next to your main source file. When compiling, LayerDocs copies its entire contents into the root of the output directory, preserving the original directory structure.
...
.box title:&#123;Why `@`?&#125;
    The `@` prefix is called [media passthrough](media-storage.qd#opting-out) and resolves to the output root.
...
Avoid placing files in `public/` that share names with LayerDocs's generated resources (e.g. `index.html`, `theme/`, `script/`). If a collision occurs, it is undefined which version ends up in the output.