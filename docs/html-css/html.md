...
On the other hand, LayerDocs strongly encourages **target agnosticism** in order to maintain rendering consistency across all supported rendering targets.
...
For this reason, LayerDocs dropped mixed content support and focused on covering the most frequent HTML workarounds with dedicated functions.
...
.example
    - A collapsible block in standard Markdown via HTML:
      ...
      ```
    - The same in LayerDocs, using the [`.collapse`](collapsible.qd) function:
      ```markdown
...
.example
    - A styled container in HTML:
      ...
      ```
    - The same in LayerDocs, using [`.container`](container.qd):
      ```markdown
