...
When running LayerDocs's [compiler](cli-compiler.qd) via `layerdocs c`, specifying the **`--pdf`** flag generates a PDF file.
...
- [Puppeteer](https://pptr.dev) (`npm install puppeteer --prefix &lt;layerdocs_dir&gt;/lib`)
...
- `QD_NPM_PREFIX`: Directory where `node_modules` should be found. Defaults to `lib` if LayerDocs was installed via a package manager or install script.
