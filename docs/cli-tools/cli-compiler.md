...
**`layerdocs c &lt;file&gt;`** is the command for compiling a main source file into a browser-renderable output.

For all available options, refer to the [CLI options](cli-options.qd) page.

## Permissions

LayerDocs uses a permission system to control what can be accessed during compilation.
...
```shell
layerdocs c main.qd --allow network --allow global-read
```
