...
Running **`layerdocs c file.qd`** compiles the given file and saves the output to file.

&gt; [!NOTE]
&gt; If the project is composed of multiple source files, the target file must be the root one, i.e. the one that [includes](including-other-layerdocs-files.qd) the other files.
...
- `-o &lt;dir&gt;` or `--out &lt;dir&gt;`: sets the directory of the output files. Defaults to `./layerdocs-output`.
...
- `--allow &lt;permission&gt;` / `--deny &lt;permission&gt;`: grants or revokes a [permission](cli-compiler.qd#permissions) (repeatable).
...
- `--timeout &lt;seconds&gt;`: sets the maximum time, in seconds, allowed for the entire execution to complete.
  `0` disables the timeout. Defaults to 30 seconds.
