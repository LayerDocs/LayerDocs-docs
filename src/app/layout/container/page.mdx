...
The **`.container`** [layerdocs-stdlib/com.layerdocs.stdlib.module.Layout/container.html](https://layerdocs.com/docs/layerdocs-stdlib/com.layerdocs.stdlib.module.Layout/container.html) function creates a highly customizable block of content, and resets the current layout rules back to normal.

## Layout reset

```layerdocs
.examplemirror \{Imagine you want to create a row with two columns, each containing a heading and some text. A beginner might try the following *wrong* approach. As explained in the [Stacks](stacks) page, stack functions rely on the strict Markdown concept of a block, an isolated chunk of the document, to determine which elements they handle.\} type:\{warning\}
    <!-- WRONG! -->
...
| `fullwidth` | Whether to take up the parent's full width. Overridden by `width`. | [`Boolean`](boolean) | False |
...
| `margin ` | Whitespace outside the content. | [`Sizes`](sizes) | None |
| `padding ` | Whitespace around the content. | [`Sizes`](sizes) | None |
| `radius` | Corner or border radius. | [`Sizes`](sizes) | None |
...
| `fontsize`, `fontweight`, `fontstyle`, `fontvariant`, `textdecoration`, `textcase` | Text transformation. See [Advanced text formatting](text) for details. | | None |
