...
Since function calls can be used inside the block argument, you can leverage use the [**`.read`**](file-data) function to load text from a file.
...
```layerdocs
.exampleoutput \{![Diagram with caption](/assets/mermaid-diagrams/with-caption.png)\}
...
.exampleoutput \{![Diagram with empty caption](/assets/mermaid-diagrams/numbered.png)\} prelude:\{To number the diagram without a caption, pass an empty string as the caption value.\}
    .mermaid caption:\{\}
        flowchart TD
            A([Start]) --> B[Enter username and password]
            B --> C\{Correct?\}
