...
You can display a table of contents through the **`.tableofcontents`** [layerdocs-stdlib/com.layerdocs.stdlib.module.Document/tableofcontents.html](https://layerdocs.com/docs/layerdocs-stdlib/com.layerdocs.stdlib.module.Document/tableofcontents.html) function.
...
|-----------|-------------|---------|---------|
| `title` | Title that precedes the table of contents. If unset, it is automatically localized. | Inline content | Automatically localized if [`.doclang`](document-metadata)'s locale is supported (e.g., `Table of Contents` for English) |
| `maxdepth` | Maximum heading level to display. For example, `maxdepth:\{2\}` collects `#` and `##` headings, but not `###` or deeper. | Integer | `3` |

```layerdocs
.exampleoutput \{!(550)[Table of contents](/logo.png)\}
...
[Layout themes](themes) greatly influence the appearance of the table of contents.

```layerdocs
.example
    !(550)[Table of contents with minimal theme](/logo.png)
...
You can customize how headings are numbered in the table of contents via the `.numbering` function. See [Numbering](numbering) for more information.
...
| `numberheading`     | Whether the heading should be [numbered](numbering) and have its position tracked in the document hierarchy.               | Boolean | `no`                                                |
...
```layerdocs
.exampleoutput \{!(550)[Table of contents with focus](/logo.png)\} prelude:\{Adding `focus:\{A.B\}` to the previous example highlights that entry:\}
