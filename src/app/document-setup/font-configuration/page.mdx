...
The **`.font`** [layerdocs-stdlib/com.layerdocs.stdlib.module.Document/font.html](https://layerdocs.com/docs/layerdocs-stdlib/com.layerdocs.stdlib.module.Document/font.html) function overrides the global font configuration.
...
> Font resources are processed by the [media storage system](media-storage). This ensures that, for example, HTML output includes local fonts in the output directory for increased portability.
>
...
- **`size`**: base font [size](sizes). Other elements, such as headings, scale accordingly. For example, `12px`.
...
```layerdocs
.exampleoutput \{![Multi-language fonts](/assets/font-configuration/multi-language.png)\}
