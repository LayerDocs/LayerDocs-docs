...
- [Figures](figure)
- Tables
- [Equations](tex-formulae)
- Code blocks
- [Footnotes](footnotes)
- Custom elements (`.numbered`)

The configuration is represented by a [Dictionary](dictionary). The following snippet shows the full configuration schema, where all entries are optional:
...
.example
    ...
    .layerdocsoutput
      !(550)[Latex theme numbering](/logo.png)

      !(550)[Latex theme table of contents](/logo.png)
...
- Use a [decorative heading](headings.qd#decorative-headings) (`#!`, `##!`, etc.), which also excludes the heading from the [table of contents](table-of-contents).
- Use the [`.heading`](headings) function with `numbered:\{no\}` for more granular control, allowing you to independently decide whether the heading appears in the table of contents.

## Figures

[Figures](figure) are numbered only if they have a **caption**, which may also be empty.

```layerdocs
.exampleoutput \{!(600)[Figure numbering with format 1.1](/logo.png)\}
    ```markdown
    .numbering
        - headings: 1.A.a
        - figures: 1.1
...
```layerdocs
.exampleoutput \{!(600)[Table numbering](/logo.png)\}
    .numbering
        - headings: 1.A.a
        - tables: 1.1
...
[Math blocks](tex-formulae) (equations) are numbered only if they have a [cross-reference ID](cross-references).

```layerdocs
.exampleoutput \{!(600)[Equation numbering](/logo.png)\}
    .numbering
        - equations: (1)
...
```layerdocs
.exampleoutput \{!(600)[Code blocks](/logo.png)\}
    .numbering
        - code: 1
...
The numbering format of [footnotes](footnotes) is flat, meaning it only considers the leftmost symbol and ignores the rest.

If not specified, footnotes format defaults to `1` (decimal).

```layerdocs
.exampleoutput \{![Footnote numbering](/assets/numbering/footnotes-roman.png)\}
    .numbering
        - footnotes: i
...
2. A [lambda](lambda) block that takes the number as an argument, formatted according to the active numbering format.
...
```layerdocs
.exampleoutput \{!(600)[Custom numbering](/logo.png)\}
    .numbering
        - headings: 1.1
        - greetings: 1.a
...
The localized name of the labeled element appears in captions if [`.doclang`](document-metadata) is set and the locale is supported. For instance, *Figure* and *Table* for the English locale, *Figura* and *Tabella* for Italian.
