...
Use **`.var \{name\} \{value\}`** to define a variable. The `value` is a [dynamic value](typing), meaning it can be of any type: text, numbers, booleans, or even complex elements like layouts.
...
For temporary variables that should only exist within a limited scope, see the [`.let`](let) function. Unlike `.var`, which creates document-wide variables, `.let` creates variables that exist only within a lambda block.
