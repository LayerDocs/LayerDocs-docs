# Let




The **`.let`** function defines a temporary variable that is accessible only within its scope. It accepts two parameters:
1. The value, of [any type](typing), to assign to the scoped variable
2. A [lambda](lambda) block that accepts one parameter (the given value)

```layerdocs
.examplemirror
    .let \{.multiply \{4\} \{2\}\}
        area:
        The area of the rectangle is .area.
        If it were a triangle, it would have been .divide \{.area\} by:\{2\}.

```
The function returns the evaluation of the lambda, so you can use it as an expression.

```layerdocs
.examplemirror
    .center
        .let \{LayerDocs\}
            name:
            .uppercase \{.name\}, .lowercase \{.name\}, .capitalize \{.name\}

```
The lambda block also accepts implicit positional parameters. See [*Lambda*](lambda) for more information.

```layerdocs
.examplemirror
    .center
        .let \{LayerDocs\}
            .uppercase \{.1\}, .lowercase \{.1\}, .capitalize \{.1\}

```