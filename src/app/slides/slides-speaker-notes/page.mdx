...
> Note: Reveal.js' speaker view requires an active [web server](cli-webserver).

### Outside speaker view

Additionally, notes may be displayed outside the speaker view, and also in exported PDF, by enabling `.slides speakernotes:\{yes\}` (see [Slides configuration](slides-configuration)).

- HTML:

  !(1000)[Notes in regular presentation](/logo.png)

- PDF:

  !(1000)[Notes in PDF](/logo.png)
