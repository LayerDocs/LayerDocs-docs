...
LayerDocs's [webserver](cli-webserver) enables direct communication between the compiler's CLI and the browser, which makes live previewing possible.

The server exposes the following endpoints:

- **`/:file`**: Serves static files relative to the target file (the `-f` option of `layerdocs start`).
...
When serving a file through the `/live/:file` endpoint, the server wraps HTML files in a simple [HTML wrapper](https://github.com/iamgio/layerdocs/tree/main/layerdocs-server/src/main/resources/live-preview/wrapper.html.template) that displays the content of `/:file` in an [iframe](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe).
...
When an iframe reloads, an unpleasant flickering effect can occur. To provide a smooth user experience, LayerDocs employs a technique called [double buffering](https://en.wikipedia.org/wiki/Double_buffering).
...
LayerDocs applies the same principle by maintaining two iframes, `A` and `B`. Suppose `A` is currently visible. When a change is detected, LayerDocs renders the updated content into `B` while `A` remains on screen. Once the rendering completes, the visible iframe switches from `A` to `B`, providing a seamless update.
